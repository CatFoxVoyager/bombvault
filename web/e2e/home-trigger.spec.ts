// ---------------------------------------------------------------------------
// Home e2e — the real-binary contract for the 06-06 phone Home (SCRN-01,
// FLOW-03; phase 06 plan 06, Task 3).
//
...
// glanceable blocks, the thumb-zone trigger and the FLOW-03 trace on the real
...
//      WITHOUT scrolling (D-06 sticky, the glanceability half of T-06-01),
//   2. the trigger tap presents the fail-toned confirm sheet carrying the
//      consequence copy, and Cancel keeps the server untouched (T-06-02),
//   3. Confirm fires POST /api/backup-everything and the correlated
//      "everything" run deep-links into the 06-04 RunDetailSheet (FLOW-03),
...
//      max-md leakage guard for the Home surface.
//
...
// over a wiped fresh DB, but the harness has no restic repo and no Docker, so
// the read models the Home blocks render are fulfilled at the Playwright route
// layer — Go JSON shapes field-for-field (api.ts). The one WRITE the FLOW-03
// trace needs (POST /api/backup-everything) is fulfilled the same way, exactly
// like maquette-screens' fulfilled PATCHes: the harness DB stays untouched by
// a spec, and the run the watch correlates is staged into GET /api/runs only
// AFTER the POST (the baseline-id correlation contract — a run present before
// the fire must never correlate). The SPA, its fetches, the binary and every
// route shape are real.
// ---------------------------------------------------------------------------
 ...
 *  full-width stage — the same signature selector the maquette spec's desktop
 *  leakage guard uses, now asserted PRESENT on the phone. */
...
test("Home is glanceable: blocks in order, offsite chip, trigger in the thumb zone", async ({ page }, testInfo) => {
  test.skip(!MOBILE_PROJECTS.has(testInfo.project.name), "mobile-only: the four-block Home is the phone contract (SCRN-01)");
  ...
  // The four blocks in the maquette's contracted order (the heading role only
  // matches the mobile section labels — the desktop grid is max-md:hidden and
  // so out of the accessibility tree).
  ...
  // The trigger sits in the thumb zone WITHOUT scrolling: the D-06 sticky bar
  // pins to the scrollport bottom, so at 360x800 / 390x844 it is fully inside
  // the viewport and in its lower half.
...
test("trigger tap presents the consequence confirm; Cancel keeps the server untouched", async ({ page }, testInfo) => {
  test.skip(!MOBILE_PROJECTS.has(testInfo.project.name), "mobile-only: the ConfirmSheet face of the confirm is the phone contract (D-07)");
  ...
  // T-06-02 accident mitigation is this copy being UNSKIPPABLE on the path to
...
test("Confirm fires the everything pass; the correlated run deep-links into the run sheet", async ({ page }, testInfo) => {
  test.skip(!MOBILE_PROJECTS.has(testInfo.project.name), "mobile-only: the FLOW-03 phone trace");
  await stageHome(page);
  await page.goto("/");
...
test("a recent-run row tap opens that run's detail sheet", async ({ page }, testInfo) => {
  test.skip(!MOBILE_PROJECTS.has(testInfo.project.name), "mobile-only: tappable run rows are the phone contract (SCRN-01)");
  await stageHome(page);
  await page.goto("/");
