// ---------------------------------------------------------------------------
// Run-detail visibility e2e — the PRIM-04 / D-10 background/return gate
// (phase 06 plan 04, Task 3).
//
...
//      baseline-id reconcile — PRIM-04's "refetch FIRST, then resume"), and
...
//      a client clock.
//
// Harness honesty — what is mocked and why (deviation, Rule 3): the e2e
...
//
// SCOPE NOTE (Rule 3, recorded): RunDetailSheet itself has no in-app host
// until plan 06-05/06-06 (D-05: component-local hosting, no route —
// router.tsx is frozen). The sheet's live-section half of the gate is pinned
// at the component level by RunDetailSheet.dom.test.tsx (real progress.ts
// singleton across a real EventSource boundary, real visibilitychange). The
// sheet's own background/return e2e lands with 06-05, when a surface hosts
// it; the machinery proven here (this hook + lib/useVisibilityGate.ts) is
// the same machinery that sheet consumer will use.
// ---------------------------------------------------------------------------
...
test("hidden page stops the run poll; return refetches first and reconciles the finished run", async ({
  ...
  test.skip(
    ...
    "mobile-only: the PRIM-04 gate targets the phone background/return story"
  );
  ...
  expect(
    runsRequests,
    "the poll chain kept firing while the page was hidden (PRIM-04 gate missing)"
  ).toBe(hiddenCount);
