...
// (SCRN-03, phase 06 plan 06-01 Task 3, D-02/D-11).
//
...
// D-11: the mode follows the pointer axis, never the width), and asserts
...
//      and check never share a tap surface — the exact hazard T-06-02
//      mitigates),
  ...
  containers: [
    {
      ...
      status: "Up 2 hours",
      ip: "172.18.0.5",
      installed: true, // the folders chip is advanced && installed — this flag opens it
...
const MOUNTS_BODY = {
  ...
  mounts: [
    ...
    },
    {
      ...
      // D-04 guard refuses (row shake + blocked warn line, no save) any
      // toggle that would leave ZERO includes for the item — with a single
 ...
 *  restructure; there is exactly one tree on this page). */
...
test("touch rows are full-size targets and the chevron zone is disjoint from the label", async ({ page }, testInfo) => {
  ...
  // (T-06-02). Disjoint x-intervals imply disjoint rectangles; 1px of slack
