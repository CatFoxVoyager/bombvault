...
export interface SelectionTreeProps {
  ...
  /** Interaction mode (phase 6, D-01/D-02): "pointer" (the default, desktop
   ...
   *  wrapper (D-01).
   *
   *  Callers derive it from useIsCoarsePointer (D-11) — NEVER from
    ...
    return (
      <Fragment key={`${spec.path}-${shakeCounts?.[spec.path] ?? 0}`}>
        <div
          ...
          className={
            touch
              ? // Touch (D-02): the full row IS the toggle target — >=44px
          ...
          onClick={
            touch
              ? // Tap-to-toggle (D-02) through the ONE toggle pipeline: the
                // exact guard Space uses (T-02-10) — unreachable rows and
        ...
        >
          ...
          {touch && spec.expandable && (
            // The touch expansion control (D-02): a real, labelled button in
