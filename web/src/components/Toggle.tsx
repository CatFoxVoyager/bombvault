  ...
  return (
    <span className={`inline-flex items-center gap-2${className ? ` ${className}` : ""}`}>
      ...
      <button
        ...
        // balloon goes.
        // Mobile hit-area extension (VERIFY-04 touch-target floor, phase 7
        // MORE-02 sweep): the track is 36x20px — far under the 44px floor —
        // but its VISUAL size is frozen by the design language, so the target
        // grows invisibly instead. Below md an empty, absolutely-positioned
        // ::after bleeds 12px past the track on every side (60x44 activation
        // box); a pseudo-element is part of its originating button, so clicks
        // anywhere in the bleed fire the switch. Desktop is untouched (the
        // max-md: prefix keeps >=48rem byte-identical), and no visible pixel
        // changes at ANY width — this is the "grow tap areas ... never visual
        // size inflation" rule satisfied literally. The 12px vertical bleed
        // reaches into the row gap above/below, which is the intended
        // forgiveness, not a collision: adjacent rows' own hit areas sit
        // underneath and still win where their content paints.
