...
//      was true when written (d68d8995) and went stale two days
...
//      markup from before the migration onto this shared component put one shared `bg-carbon-surface2` behind the
...
//      variants since item 6's generalization; see item 6). Live-review follow-up:
...
//
//      Corrected (jdp, live review, explicit): the first pass gave
...
//      shared padded groove; is what the idle-fill pass specifically built ("idle
...
//      reason, and item 6 made that literal; it is now the
...
//      applied; a later pass then found the same reasoning was never sound for
...
//   5c. MIN_PINNED_WIDTH (a live-review refinement on top of 5b/5):
...
//   6. `variant="track"`; added during the follow-up pass, removed again before it
//      landed. The experiment added a third variant for the compact, repeated-per-card selectors
...
//        Reversed (jdp, live review, explicit): "Die kleinen
...
export interface SelectorItem {
   ...
   *  Local/Remote pair, GlimStone follow-up round; "replace the
...
interface SelectorCommon {
   ...
   *  ("soll das nicht besser ein horizontaler selektor sein oder
   ...
   *  the first did not (jdp, on the three label-mode selectors in
   ...
   *  file header item 6): pin every segment to the widest one's own
 ...
 * upstream's plus the Platform sub-section's own picker) spread across the
 * tabs, so starts are assigned per tab: no two simultaneously-visible
...
// refinement on top of item 5b's own fix: "Die horizontalen
...
function SelectorTab({
  ...
  // in the app, which is what jdp reported ("die button Graceful,
...
export function Selector(props: SelectorProps) {
  ...
  //   Broken (jdp, live review; "Dunkel"
  // rendering as "Du..." in the Theme Card, "Leicht" as "Lei..." in the Shape
  // Card): once Settings.tsx's own `inline-flex self-start` wrapper
  ...
  return (
    <div
      ...
      // as first shipped (file header item 6 (b)): the groove has to be
      ...
      // how the small variant shipped invisible in its first cut.
      ...
      // segments); the first cut's own re-derived `[0.15rem]` left a 2.4px ring,
      ...
      //   `flex-wrap`, not the `flex-nowrap` "well" carried before the generalization:
      ...
      {items.map((item, i) => {
        ...
        const cls = [
          ...
          on
            ...
            : // "well", both scales (jdp, live review: "Die kleinen
              // Selektoren sollen so aussehen wie die grossen! Die nicht
          ...
          // including an unpinned "well" one, which item 6 newly makes
