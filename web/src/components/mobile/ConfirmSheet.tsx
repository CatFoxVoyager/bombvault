...
// half on useIsDesktop: desktop gets ConfirmDialog byte-identically (D-07's
// hard guarantee — not one class token may move), narrow viewports get this
// sheet. Same promise, same settle paths, same translated strings — only the
// surface changes.
//
...
// PRIM-01 primitive), so this file is props in, an element tree out. The
...
//     position — per UI-SPEC PRIM-03: "safe action = neutral, rendered as
//     the primary-position (thumb-default) control". BottomSheet's open
//     effect lands initial focus on the header close button (the safe
//     outcome), and nothing here re-focuses the destructive control: D-07's
//     "never default-focused" holds without an autoFocus anywhere.
//
// Known gap, deliberate: the desktop card is aria-describedby its message
// (confirmdialog-message); BottomSheet has no described-by slot yet, so the
// sheet announces title + visible message only. Adding the slot is a
// primitive change owned by the sheet roadmap, not this consumer.
// ---------------------------------------------------------------------------
    ...
    <BottomSheet open onClose={onCancel} title={title} tone={tone}>
      ...
      <div className="flex flex-col gap-3 px-4 py-3">
        ...
        <Button
          ...
          // its glyph — ConfirmDialog passes the identical null. D-07: the
