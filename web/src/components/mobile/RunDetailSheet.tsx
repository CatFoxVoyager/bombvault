...
// ---------------------------------------------------------------------------
// RunDetailSheet — SCRN-05, the full-screen run-detail sheet (phase 06 plan 04,
// D-05).
//
// Hosted COMPONENT-LOCALLY by whichever surface owns the run row (06-05
// container/file-set surfaces, 06-06 Home): `<RunDetailSheet run={...} open
...
// which is the D-05 contract.
//
// RECORDED DEVIATION vs SCRN-05's verbatim wording (the Frozen-API Data
// Adaptations contract, 06-UI-SPEC.md — binding for this milestone): SCRN-05's
// "new / changed / unchanged" stat triad and its per-file exclusion-reason log
// lines have NO source on the frozen Run record (web/src/lib/api.ts:341 —
...
//     in this plan's Task 1 — one implementation, two surfaces)
//   - shell: BottomSheet fullHeight + footer (plan 06-02), three close paths
...
//   - browse: SnapshotFileTree behind a tonal row (Recovery.tsx:606 /
//     Files.tsx:345 parent-fetch precedent; RestorePanel.tsx is the local
//     container twin)
...
export interface RunDetailSheetProps {
   ...
   *  extrapolates completion from clocks (D-10). */
  run: Run;
...
export function RunDetailSheet({ run, open, onClose }: RunDetailSheetProps) {
  ...
  // Lazy parent-fetch on expand — the RestorePanel.tsx:99-112 shape (list →
