...
// lost — no bar slot is active there, the sheet's row is. Active detection
// rides NavLink's isActive, the same className-by-isActive shape the rail's
// NavItem and the bar's slots use — no parallel active-state bookkeeping.
//
// Tap-on-active parity with the bar (SHELL-02): tapping the ALREADY-current
// row scrolls the scroller back to the top — the mechanism is Layout's
// <main id="bv-main"> scroll, passed down through BottomNav, so this file
// never queries the DOM for the scroller itself.
//
// Sign-out (T-05-02, locked decision from 05-CONTEXT.md): the sidebar
// footer's row re-expressed at the BOTTOM of the sheet, visually muted
// (muted text token, hairline separation, power glyph) and with NO
...
// screen back). Gated by the same authEnabled flag as the desktop footer, so
// an instance without a password offers the row on NEITHER surface.
// ---------------------------------------------------------------------------
...
export interface MoreSheetProps {
   ...
   *  the desktop Sidebar footer (T-05-02: one flag, both surfaces). */
  authEnabled: boolean;
...
// One destination row: 52px minimum height (min-h-[3.25rem], UI-SPEC's
// maquette iOS list cell) — comfortably over the 44px touch floor; the
// padding never carries the floor, the min-height does.
...
export function MoreSheet({ open, onClose, settings, authEnabled, scrollMainToTop }: MoreSheetProps) {
  ...
  // back. NO confirmation dialog: a password IS the confirmation (locked
  // decision, 05-CONTEXT.md).
  const signOut = async () => {
