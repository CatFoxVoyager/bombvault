...
// bar (D-03). Per the plan: assert CLASS, not computed layout — jsdom has no
// layout engine, so "sticky works" is pinned as (a) the sticky/bottom-0
// classes present and (b) the fixed-positioning anti-pattern ABSENT (the
// locked Phase 5 discipline), plus the chrome trio (sidebar surface, top
// hairline, safe-area bottom padding) that the BottomNav precedent shares.
// ---------------------------------------------------------------------------
...
describe("StickyActionBar", () => {
  ...
  it("is a normal-flow sticky element — sticky bottom-0 classes present, position:fixed absent", () => {
    ...
    // keyboard mechanism; Phase 5 decision restated at the component header).
  ...
  it("appends caller className after its own (callers add md:hidden-style visibility gates)", () => {
    const { container } = render(
