...
// ---------------------------------------------------------------------------
// TapPopover — PRIM-02, the anchored tap-popover primitive (D-09).
//
...
//   - Focus is light (D-09: a popover is NOT a modal): on open, focus moves
...
//     so there is no dimming. It is a REAL event-consuming layer (threat
//     T-06-03): outside taps land on it and die there, never reaching the
...
// translated by the caller (i18n.ts + locales/ are single-owner in Wave 1 —
// no new keys may be minted here).
// ---------------------------------------------------------------------------
...
export function TapPopover({
  ...
  // Light focus (D-09), BottomSheet's open-effect shape minus the trap: move
      ...
      {createPortal(
        <>
          {/* The consuming dismissal layer: transparent (NO scrim tint — a
              ...
              real hit target so outside taps die here (T-06-03). Deliberately
