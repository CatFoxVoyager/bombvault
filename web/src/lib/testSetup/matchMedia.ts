...
// getResolvedTheme()/onSystemThemeChange() — plus, from phase 5 on,
// lib/useMediaQuery.ts during the Layout render — call it the moment the
// component they serve mounts. This module is wired via test.setupFiles in
// vitest.config.ts and landed in the SAME change as useMediaQuery (the
// Pitfall-1 coupling): the first suite that renders the real Layout in jsdom
// (app/Layout.displayPrefs.dom.test.tsx) would otherwise throw
// "window.matchMedia is not a function" the moment the chrome switch hooks
// in.
//
...
function stubbedMatchMedia(query: string): MediaQueryList {
  ...
  const mql = {
    ...
    // APIs stay consistent, mirroring the theme.ts:78-87 shape the hook uses.
