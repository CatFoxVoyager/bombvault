...
export function useConfirm() {
  const { t } = useT();
  // PRIM-03 / D-07: the presentation half swaps at the ONE width breakpoint —
  // ConfirmDialog (the desktop card, byte-identical to before this hook grew
  // the branch) at/above 48rem, ConfirmSheet (the fail-toned bottom sheet,
  // destructive control on top, safe cancel in the thumb-default bottom
  // slot) below it. Nothing else about the contract moves: same confirm()
  // promise, same settle paths, same translated strings, zero per-call-site
  // changes.
