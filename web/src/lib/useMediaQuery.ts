...
//
// POINTER_COARSE_QUERY (phase 6, D-11) is a DIFFERENT axis and deliberately
// independent of the width one: it answers "does the primary input lack a
// fine pointer" (touch), not "is the window wide". Width decides chrome
// (Layout's Sidebar vs bottom bar); pointer capability decides interaction
// (SelectionTree's tap-to-toggle touch mode, D-01/D-02). Collapsing the two
// would hand a landscape phone (>=48rem wide, still coarse-pointer) the
// desktop tree whose hover affordances and 24px targets a finger cannot use
// — research Pitfall 6 — and would hand a hybrid touchpad laptop a touch
// tree it did not ask for. This file stays the one home of BOTH literals so
// the "no second copy" discipline covers each; the guard test pins
// DESKTOP_QUERY as the only WIDTH literal and the exact coarse-pointer
// string alongside it.
//
...
// ---------------------------------------------------------------------------
// Pointer-capability axis (D-11) — an additive twin of the width hook above,
// same lazy-MediaQueryList / useSyncExternalStore / Safari<14 shape, over a
// second module-level list so the two queries can never share state.
// ---------------------------------------------------------------------------
...
function getCoarseSnapshot(): boolean {
  ...
  // interaction mode they assert (phase 6 D-01/D-11). A real browser always
  // answers the query itself.
  return mql ? mql.matches : false;
}

/** True when the primary pointer is coarse (touch input). Windowless/jsdom
 *  snapshot: false (pointer mode). Drives SelectionTree's interactionMode —
 *  NEVER derive it from useIsDesktop (D-11: width is the chrome axis only). */
